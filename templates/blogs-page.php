...
// Get the current page ID for metabox data.
$current_page_id = get_the_ID();

// --- Banner Slider ---
$slider_post_ids = get_post_meta($current_page_id, '_blogs_slider_posts', true);
if (is_array($slider_post_ids) && !empty($slider_post_ids)) {
	$slider_args = array(
		'post_type'      => 'post',
		'post_status'    => 'publish',
		'posts_per_page' => 5,
...
// --- Blog Posts List (initial page 1) ---
				...
				<div class="blogs-page-filter">
					<div class="blogs-page-filter__item">
						<div class="blogs-page-filter__buttons" id="category-buttons">
							<button class="blogs-page-filter__btn active" data-category=""><?php esc_html_e('All', 'nghaihoang9x'); ?></button>
							<?php foreach ($all_categories as $cat) : ?>
								<button class="blogs-page-filter__btn" data-category="<?php echo esc_attr($cat->slug); ?>"><?php echo esc_html($cat->name); ?></button>
							<?php endforeach; ?>
						</div>
					</div>
				</div>
